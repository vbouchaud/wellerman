...
PWD := $(shell pwd)
NOW := $(shell date +%s)
APPNAME := $(notdir $(basename $(shell git remote get-url --push origin || echo unsupported)))
BIN ?= $(APPNAME)
...
.PHONY: fmt fmt-check vet test test-coverage cover tidy install hooks help clean default
default: all

## Install git hooks for post-checkout & pre-commit
hooks:
	@cp -f ./githooks/post-checkout .git/hooks/
	@cp -f ./githooks/pre-commit .git/hooks/
